...
class RegistroController
{
    ...
    public static function conferencias(Router $router)
    {
        ...
        // Redireccionar a boleto virtual en caso de finalizado el registro
        if (isset($registro->regalo_id) && $registro->paquete_id === "1") {
            header('Location: /boleto?id=' . urlencode($registro->token));
            return;
        }

        $registro = Evento::ordenar('hora_id', 'ASC');

        $eventos_formateados = [];
        foreach ($registro as $evento) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $eventos_array = explode(',', $_POST['eventos']);
            if (empty($eventos_array)) {
            ...
            foreach ($eventos_array as $evento) {
                ...
                // Comprobar euq el elemento exista
